...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize create practice modal
    setupCreatePracticeModal();
...
// --- CREATE PRACTICE MODAL FUNCTIONALITY ---
let currentCreationStep = 'step-1';
let selectedTemplate = null;
let newlyCreatedPractice = null;

function setupCreatePracticeModal() {
    const createPracticeBtn = document.getElementById('create-practice-btn');
    const createPracticeModal = document.getElementById('create-practice-modal');
    const closeCreateModal = document.getElementById('close-create-modal');
    
    // Step elements
    const step1 = document.getElementById('creation-step-1');
    const aiStep = document.getElementById('ai-creation-step');
    const templateStep = document.getElementById('template-selection-step');
    const loadingStep = document.getElementById('creation-loading-step');
    const successStep = document.getElementById('creation-success-step');
    
    // Navigation buttons
    const createWithAiBtn = document.getElementById('create-with-ai-btn');
    const useTemplateBtn = document.getElementById('use-template-btn');
    const backBtns = document.querySelectorAll('.back-btn');
    const generatePracticeBtn = document.getElementById('generate-practice-btn');
    
    // Success buttons
    const editNewPracticeBtn = document.getElementById('edit-new-practice-btn');
    const createAnotherBtn = document.getElementById('create-another-btn');
    
    // Modal open/close
    createPracticeBtn?.addEventListener('click', openCreatePracticeModal);
    closeCreateModal?.addEventListener('click', closeCreatePracticeModal);
    createPracticeModal?.addEventListener('click', (e) => {
        if (e.target === createPracticeModal) closeCreatePracticeModal();
    });
    
    // Step navigation
    createWithAiBtn?.addEventListener('click', () => showCreationStep('ai'));
    useTemplateBtn?.addEventListener('click', () => showCreationStep('template'));
    backBtns.forEach(btn => btn.addEventListener('click', () => showCreationStep('step-1')));
    
    // AI generation
    generatePracticeBtn?.addEventListener('click', handleGeneratePractice);
    
    // Success actions
    editNewPracticeBtn?.addEventListener('click', editNewlyCreatedPractice);
    createAnotherBtn?.addEventListener('click', createAnotherPractice);
    
    // Load templates when template step is shown
...
function openCreatePracticeModal() {
    ...
    modal.classList.remove('hidden');
    showCreationStep('step-1');
}

function closeCreatePracticeModal() {
    const modal = document.getElementById('create-practice-modal');
    modal.classList.add('hidden');
    resetCreatePracticeModal();
}

function resetCreatePracticeModal() {
    currentCreationStep = 'step-1';
    selectedTemplate = null;
    newlyCreatedPractice = null;
    
    // Clear AI form
    const aiFocus = document.getElementById('ai-focus');
    const aiDuration = document.getElementById('ai-duration');
    if (aiFocus) aiFocus.value = '';
    if (aiDuration) aiDuration.value = '60';
    
    // Reset generate button
    const generateBtn = document.getElementById('generate-practice-btn');
    if (generateBtn) {
        generateBtn.disabled = false;
        generateBtn.innerHTML = 'Generate Practice Plan';
    }
}

function showCreationStep(step) {
    // Hide all steps
    const steps = ['creation-step-1', 'ai-creation-step', 'template-selection-step', 'creation-loading-step', 'creation-success-step'];
    ...
    switch (step) {
        case 'step-1':
            targetStepId = 'creation-step-1';
            break;
        case 'ai':
            targetStepId = 'ai-creation-step';
            break;
        case 'template':
            targetStepId = 'template-selection-step';
            break;
        case 'loading':
            targetStepId = 'creation-loading-step';
            break;
        case 'success':
            targetStepId = 'creation-success-step';
            break;
    }
...
async function handleGeneratePractice() {
    ...
    const duration = parseInt(aiDuration?.value) || 60;
    
    if (!focus) {
        alert('Please describe the main focus of your practice.');
        aiFocus?.focus();
        return;
    }
    ...
    showCreationStep('loading');
    
    try {
        // Use AI assistant to generate practice
        const generatedPractice = await aiAssistant.generatePracticeFromPrompt(focus, duration);
        
        // Find next available practice ID
        const nextId = getNextAvailablePracticeId();
        
        // Create practice object
        const newPractice = {
            id: nextId,
            title: generatedPractice.title || `AI Practice: ${focus}`,
            totalTime: duration,
            warmup: generatedPractice.warmup || {
                title: "Dynamic Warm-up",
                desc: "Get players moving and ready for practice.",
                duration: 5
            },
            stationTime: generatedPractice.stationTime || Math.floor(duration * 0.6),
            stationInstructions: generatedPractice.stationInstructions || "Rotate through stations focusing on the practice goals.",
            stations: generatedPractice.stations || [],
            finisher: generatedPractice.finisher || {
                title: "Fun Finisher",
                desc: "End practice on a high note with a fun activity.",
                duration: 10
            },
            wrapup: generatedPractice.wrapup || {
                title: "Circle Time",
                desc: "Gather players to review what we learned today.",
                duration: 5
            },
            homework: generatedPractice.homework || {
                title: "Practice at Home",
                desc: "Continue working on today's skills with family.",
                video: ""
            },
            // Metadata
            createdAt: new Date(),
            createdBy: currentUser?.displayName || currentUser?.email || 'AI Assistant',
            lastUpdatedAt: new Date(),
            lastUpdatedBy: currentUser?.displayName || currentUser?.email || 'AI Assistant',
            source: 'ai_generated',
            aiPrompt: focus
        };
        
        // Save to Firestore
        const docRef = await addDoc(collection(db, "practices"), newPractice);
        newlyCreatedPractice = { ...newPractice, docId: docRef.id };
        
        // Log the creation
        await logPracticeChange(nextId, newPractice.title, 'ai_created');
        
        // Show success
        showCreationStep('success');
...
function getNextAvailablePracticeId() {
    const existingIds = practices.map(p => p.id).sort((a, b) => a - b);
    
    // Find first gap in sequence or return next number
...
async function loadPracticeTemplates() {
    const templateGrid = document.getElementById('template-grid');
    if (!templateGrid) return;
    
    // For now, create some sample templates
    // In the future, these would come from a templates collection
    const sampleTemplates = [
        {
            id: 'fundamentals',
            name: 'Fundamentals Focus',
            description: 'Basic skills and fun',
            category: 'beginner',
            duration: 60
        },
        {
            id: 'defense',
            name: 'Defense First',
            description: 'Fielding and throwing',
        ...
        },
        {
            id: 'hitting',
            name: 'Hitting Power',
            description: 'Batting and contact',
            category: 'intermediate',
            duration: 60
        },
        {
...
async function selectTemplate(template) {
    ...
    try {
        ...
        // For now, use existing practice as template base
        // In the future, this would load from a templates collection
        const basePractice = practices[0]; // Use first practice as template
        
        const newPractice = {
            ...basePractice,
            id: nextId,
            title: `${template.name} Practice`,
            totalTime: template.duration,
            // Metadata
            createdAt: new Date(),
            createdBy: currentUser?.displayName || currentUser?.email || 'Template System',
            lastUpdatedAt: new Date(),
            lastUpdatedBy: currentUser?.displayName || currentUser?.email || 'Template System',
            source: 'template',
            templateId: template.id
        };
        
        // Remove the docId from the base practice
        delete newPractice.docId;
        ...
        // Log the creation
        await logPracticeChange(nextId, newPractice.title, 'template_created');
        
        // Show success
        showCreationStep('success');
        
    } catch (error) {
        console.error('Error creating practice from template:', error);
        alert('Failed to create practice from template. Please try again.');
        showCreationStep('template');
    }
}

function editNewlyCreatedPractice() {
    if (newlyCreatedPractice) {
        closeCreatePracticeModal();
        editPractice(newlyCreatedPractice.docId);
    }
}

function createAnotherPractice() {
    showCreationStep('step-1');
    resetCreatePracticeModal();
}
